...
int box_x=0,box_y[]={8,20,18,7,10,5,0},space=8;//0<=y<=(32-space)
...
//    if(thisNote<0)thisNote=47;
 ...
 void gameover(){ //遊戲結束
   ...
   if(flag==2) {
    ...
    display.display();
    for(int i=1;i<=1 && ggbgm;i++){
        int noteDuration = 10000 / noteDurations[i];
        tone(buzzer, melody[24], noteDuration);
        int pauseBetweenNotes = noteDuration * 1.30;
        delay(pauseBetweenNotes);
    }
   }
...
//被拆到movement和draw了
//void bgm(){  
//    for (int thisNote = 47; thisNote >=0 ; thisNote--) {
//
